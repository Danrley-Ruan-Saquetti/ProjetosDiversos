...
function sat(polygonA, polygonB) {
    ...
    for (var i = 0; i < polygonA.edge.length; i++) {
        perpendicularStack.push(new xy(-polygonA.edge[i].y, polygonA.edge[i].x))
    }
    //Work out all perpendicular vectors on each edge for polygonB
    for (var i = 0; i < polygonB.edge.length; i++) {
        perpendicularStack.push(new xy(-polygonB.edge[i].y, polygonB.edge[i].x))
    }
    ...
    for (var i = 0; i < perpendicularStack.length; i++) {
        ...
        for (var j = 0; j < polygonB.vertex.length; j++) {
            dot = polygonB.vertex[j].x * perpendicularStack[i].x + polygonB.vertex[j].y * perpendicularStack[i].y //Then find the dot products with the highest and lowest values from polygonB.
